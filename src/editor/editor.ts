...
export class Editor extends Subject{
    ...
    private _drawPointSymbol: SimplePointSymbol = new SimplePointSymbol();

    //自定义右键菜单
    private _container: any;
    private _contextMenu: any;
    private _menuFinishEditing: any;
    private _menuStopEditing: any;
    private _menuDeleteVertex: any;
    ...
    _deleteVertex() {
        if (!this._editingVertex) return;
        const vertex = this._editingVertex;
        const geometry: Geometry = this._editingFeature.geometry;
        if (geometry instanceof Polygon) {
            ...
            const point: Point = vertex.geometry;
    ...
    _drawMiddlePoint(line, closed) {
        ...
        line.forEach((lnglat, index) => {
            if (closed || index < line.length - 1) {
                ...
                const p: Point = new Point(lng, lat);
        ...
        if (this._drag.flag) {
            ...
            if (this._editingFeature.geometry instanceof Point) {
                ...
                if (this._drag.middle) {
                    const polyline: Polyline = this._editingFeature.geometry;
                    const point: Point = this._drag.middle.geometry;
